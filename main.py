...
import threading#导入threading模块，用于多线程，解决主界面卡顿问题。
def valid_usb_drive(drive):#这个函数负责设置函数，检查输入的盘符是否为有效的U盘路径
    if len(drive) == 3 and drive[0].isalpha() and drive[1] == ':' and drive[2] == '\\':#用于验证用户行为，确保输入的盘符格式正确
        return os.path.exists(drive)
    return False#返回 False
def is_process_in_efficiency_mode(pid):#这个函数负责检查指定 PID 的进程是否处于效率模式
    ...
    try:#获取进程信息
        ...
        if result:#判断API调用是否成功
            ...
            return False
    except Exception as e:#捕获异常：无法检查效率模式
        print(f"检查效率模式失败: {e}")#打印错误信息
        return False
def get_usb_holding_processes(drive):#这个函数负责获取占用 U 盘的进程列表
...
def check_usb_usage():#这个函数负责检查 U 盘使用情况
    usb_drive = entry.get().strip()#获取用户输入的 U 盘盘符
    usb_drive = usb_drive.replace('：', ':')#替换中文冒号为英文冒号
    usb_drive = usb_drive.upper()#将用户盘符转换为大写
    #根据不同输入情况构建标准盘符格式
    if len(usb_drive) == 1 and usb_drive.isalpha():#用户只输入了盘符字母
        usb_drive = usb_drive + ':\\'
    elif len(usb_drive) == 2 and usb_drive[0].isalpha() and usb_drive[1] == ':':#用户输入了盘符字母和冒号
        usb_drive = usb_drive + '\\'
    elif len(usb_drive) == 3 and usb_drive[0].isalpha() and usb_drive[1] == ':' and usb_drive[2] == '\\':#格式完整无误
        usb_drive = usb_drive
    else:#其他意外情况
        if len(usb_drive) == 2 and usb_drive[0].isalpha() and usb_drive[1] != ':':#用户忘记打冒号了
            usb_drive = usb_drive[0] + ':\\'
        else:#利用前面的函数检查输入的 U 盘盘符是否为有效
            messagebox.showerror("盘符无效", "请输入有效的U盘盘符（例如 G:）")#如果用户输入了错误的信息,显示错误信息
            return False#返回 False
    result_text.config(state="normal")#启用结果文本框
    result_text.delete(1.0, tk.END)#清空结果文本框
    result_text.insert(tk.END, "正在检查，请稍候...\n")#显示正在检查的提示信息
    result_text.config(state="disabled")#禁用结果文本框,避免用户修改结果
    root.update()#更新界面
    threading.Thread(target=check_usb_usage_thread, args=(usb_drive,), daemon=True).start()#创建线程，调用 check_usb_usage_thread 函数，传入 U 盘盘符
    return True#返回 True
def update_result_text(processes,usb_drive):#这个函数负责更新结果文本框
    result_text.config(state="normal")#启用结果文本框
    result_text.delete(1.0, tk.END)#清空结果文本框
    if not processes:#如果没有进程占用 U 盘
        result_text.insert(tk.END, "当前没有进程占用该盘。\n")#显示没有进程占用 U 盘的提示信息
    else:#如果有进程占用 U 盘
        for p in processes:#遍历进程列表
            if p['efficiency']:#如果进程处于效率模式
                message = f"进程 {p['name']} (PID: {p['pid']}) 正在占用 U 盘，并且处于效率模式。\n"#创建提示信息
                message += "若要弹出 U 盘，需要先退出该进程的效率模式或结束该进程。"#添加提示信息
...
def check_usb_usage_thread(usb_drive):#多线程，解决主界面卡顿问题
    processes = get_usb_holding_processes(usb_drive)#获取占用 U 盘的进程列表
    root.after(0,update_result_text, processes,usb_drive)#更新结果文本框，在主线程中执行，包括进程列表和 U 盘盘符
root = tk.Tk()#创建主窗口（即GUI界面）
root.title("U盘占用检查")#设置窗口标题
...
label = tk.Label(root, text="请输入 U 盘盘符（例如 G:/，缺少字符也可）:", font=("微软雅黑", 10))#创建标签，包括标题和字体
